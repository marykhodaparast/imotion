                        ...
                        <tbody>
                            ...
                            @foreach ($user_slots as $date => $slots)
                                <tr>
                                    ...
                                    @for ($j = 1; $j <= 20; $j++)
                                        @if ((jdate()->format('w') + $i) % 7) @if (!empty($slots))
                                            ...
                                            @elseif($slots["slot-".$j]["seat_count"] == null
                                                && $slots["slot-".$j]["is_mine"] == null)
                                                <td
                                                class="{{ $slots['is_mine'] == 1 ? 'table_inactive' : 'cursor_pointer hoverable' }}"
                                                data-date="{{ jdate()->addDays($i - 1)->format('Y-m-d') }}"
                                                data-nameOfDay="{{ jdate()->addDays($i - 1)->format('l') }}"
                                                >
                                                <div class="row width-50 mx-auto">
                                            ...
                                            @elseif($slots["slot-".$j]["seat_count"] == 3)
                                                <td
                                                class="{{ $slots['slot-' . $j]['is_mine'] == 1 ? 'cursor_pointer hoverable' : '' }}"
                                                data-date="{{ jdate()->addDays($i - 1)->format('Y-m-d') }}"
                                                data-nameOfDay="{{ jdate()->addDays($i - 1)->format('l') }}"
                                                >
                                                <div class="row width-50 mx-auto">
                                                @for ($k = 0; $k < 3; $k++) <i
                                                    class="fas fa-user danger"></i> @endfor
                </div>
                </td>
                @endif
            @else
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('td').on('click', function() {
                ...
                if (nameofday != 'جمعه' && !$(this).hasClass('table_inactive') && !$(this).hasClass(
                        'bg-danger')) {
